...
function App() {
  ...
  const [loadedLines, setLoadedLines] = useState({}); // Suivi des lignes cochées
  ...
  const calculateCargoProgression = (steps) => {
    ...
    return steps.slice(0, -1).map((step, i) => {
      step.lines.forEach((line) => {
        if (!loadedLines[line.lineId]) return; // Ignorer si non chargé
        const match = line.text.match(/(Chargez|Déchargez) (\d+) SCU/);
        const quantity = match ? parseInt(match[2]) : 0;
        if (line.text.startsWith('Chargez')) currentCargo += quantity;
        if (line.text.startsWith('Déchargez')) currentCargo -= quantity;
      });
      return {
  ...
  const customSelectStyle = {
    ...
    option: (provided) => ({
      ...provided,
      color: 'black',  // Change color of options to black
    }),
  };
          ...
          {summaryLines.length
            ? (() => {
                ...
                return summaryLines.map((step, i) => (
                  <li key={i} className="summary-step">
                      ...
                      {step.lines.map((line, j) => (
                        <li key={j} className="summary-line">
                          ...
                          <div className="move-buttons">
                            <button onClick={() => moveLine(i, -1)} className="move-up-button" title="Monter"></button>
                            <button onClick={() => moveLine(i, 1)} className="move-down-button" title="Descendre"></button>
                          </div>
